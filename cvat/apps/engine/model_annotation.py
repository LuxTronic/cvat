...
def _call_provider(
    *, content: list[dict], model_name: str, timeout: int
    ...
    base_url = str(
        provider_settings.get("URL", "https://generativelanguage.googleapis.com")
    ).rstrip("/")
...
def _has_any_annotations_on_frame(task_id: int, frame_id: int) -> bool:
    return (
        ...
        or TrackedShape.objects.filter(
            track__job__segment__task_id=task_id, frame=frame_id
        ).exists()
    )
...
def run_model_annotation_task(
    ...
    raw_response_text, parsed_json = _call_provider(
        content=content, model_name=model, timeout=timeout
    )
...
class ModelAnnotatePermission(TaskPermission):
    """
    Task policy for /api/model/annotate.

    The endpoint is not a detail route, so the generic enforcement path hands us
    obj=None; resolve the target task from the request body instead, otherwise
    the task policy is evaluated against a null resource and denies everyone but
    admins.
    """

    @classmethod
    def create(
        cls, request: ExtendedRequest, view, obj: Task | None, iam_context: dict | None
    ) -> list:
        if obj is None and view.action == "annotate":
            obj = _get_target_task(request)
            iam_context = get_iam_context(request, obj)

        return super().create(request, view, obj, iam_context)

    @classmethod
    def _get_scopes(cls, request: ExtendedRequest, view, obj: Task | None) -> list:
        if (view.action, request.method) == ("annotate", "POST"):
            return [cls.Scopes.UPDATE_ANNOTATIONS]

        return super()._get_scopes(request, view, obj)


def _get_target_task(request: ExtendedRequest) -> Task:
    task_id = request.data.get("task_id") if isinstance(request.data, dict) else None
    if task_id is None:
        raise ValidationError("task_id is required")

    try:
        return Task.objects.select_related("project", "organization").get(pk=task_id)
    except (Task.DoesNotExist, TypeError, ValueError) as ex:
        raise ValidationError(f"Unknown task_id: {task_id!r}") from ex
...
class ModelAnnotationViewSet(viewsets.GenericViewSet):
    ...
    iam_permission_class = ModelAnnotatePermission
    ...
    def annotate(self, request: ExtendedRequest):
        ...
        # Access to the task was already enforced by ModelAnnotatePermission.
        task = _get_target_task(request)
