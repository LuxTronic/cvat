...
def patch_job_data(
    ...
    def _payload_supervision_frames(payload: AnnotationIR | dict) -> tuple[set[int], set[int]]:
        if isinstance(payload, AnnotationIR):
            annotation_ir = payload
        else:
            annotation_ir = AnnotationIR(annotation.db_job.segment.task.dimension, payload)
        ...
        for shape in annotation_ir.shapes:
