...
const MODEL_SOURCE_LABELS: Record<string, string> = {
    registry_active_slot: 'active slot',
...
const componentShortcuts = {
    SWITCH_AUTO_ANNOTATE_STANDARD_CONTROLS: {
        ...
        // 'q' is taken by SWITCH_OCCLUDED in the objects sidebar, which the
        // conflict detector treats as an overlapping scope.
        sequences: ['a'],
...
function AutoAnnotateControlComponent(): JSX.Element {
    ...
    const [modelOptions, setModelOptions] = useState<AutoAnnotateModel[]>([]);
    ...
    const selectedModel = useMemo(
        () => modelOptions.find((model): boolean => model?.id === selectedModelID),
            ...
            try {
                ...
                if (!cancelled) {
                    ...
                    setSelectedModelID((current): string => (
                        current && models.some((model): boolean => model?.id === current) ?
                            current :
                            ''
                    ));
                }
    ...
    const handleModelAnnotate = useCallback(async () => {
        ...
        try {
            ...
            const response = await core.server.request<{ data?: { rq_id?: string } }>(
        ...
        <div style={{ minWidth: 280 }}>
            <div style={{ textAlign: 'center' }}>
                ...
                >
                    ...
                    {modelOptions.map((model): JSX.Element => (
                            ...
                            <div style={{ display: 'grid', gap: 2 }}>
                                ...
                                <span style={{ fontSize: 11, color: '#8c8c8c' }}>
                                    {[
                                        ...
                                        MODEL_SOURCE_LABELS[model.source_kind ?? ''] ?? 'legacy',
